    ...
    module.controller('directorController', function($scope, $http) {
        ...
        $http.get('php/buscarCursosDirector.php').success(function(data) {
            ...
            for (var i = $scope.NombreCarrera1.length - 1; i >= 0; i--) {
                ...
                $scope.cursosDB.push({
                    CodCur: $scope.NombreCarrera1[i].id_curso,
        ...
        $scope.agregarProfeEstudSelect = function() {
            ...
            for (var i = $scope.NombreProfes.length - 1; i >= 0; i--) {
                archivos.ProfesoresNormales.push({
                    "Profesor": $scope.NombreProfes[i]
                })
            };

            for (var i = $scope.NombreEstuds.length - 1; i >= 0; i--) {
                archivos.EstudiantesNormales.push({
                    "Estudiante": $scope.NombreEstuds[i],
                    "Correo": $scope.NombreEstuds1[i].correo
                })
            };

            for (var i = $scope.NombreCarrera.length - 1; i >= 0; i--) {
                archivos.CursosDisponilbes.push({
                    "Curso": $scope.NombreCarrera[i]
                })
            };
                ...
                $http.get('php/buscarGrupoEstudiantes.php').success(function(dato) {
                    ...
                    for (var i = est.length - 1; i >= 0; i--) {
                        estudiantesTemporales.push({
                            NombreEstudiante: est[i].NombreEstudiante,
                    ...
                    $http.get('php/buscarGruposProfesor.php').success(function(date) {
                        ...
                        for (var i = tal.length - 1; i >= 0; i--) {
                            profesoresTemporales.push({
                                NombreProfesor: tal[i].profesor,
                                Rol: tal[i].nombre,
                    ...
                    for (var i = $scope.carreras.length - 1; i >= 0; i--) {
                        ...
                        for (var j = $scope.conten.length - 1; j >= 0; j--) {
                            if ($scope.conten[j].nombreCarrera == $scope.carreras[i].nombre) {
                                carreraTemp.CursosAsignados.push({
                                    "curso": $scope.conten[j].nombreCurso
                                })
                            };
                        };
        ...
        $scope.delCurso = function(valores) {
            $scope.cursosAdd = [];
            var carreraSeleccionada = $scope.valor0;
            $http.get('php/buscarCursosDirector.php').success(function(data) {
                $scope.care = data;
                for (var i = $scope.NombreCarrera1.length - 1; i >= 0; i--) {
                    $scope.cursosAdd.push({
                        CodCur: $scope.care[i].id_curso,
                        Nombre: $scope.care[i].nombre
                    })
                };

                $http.get('php/buscarCarreraDirector.php').success(function(date) {
                    $scope.delCarreras = date;
                    var idCarrera = 0;

                    for (var te = $scope.delCarreras.length - 1; te >= 0; te--) {

                        if ($scope.delCarreras[te].nombre === carreraSeleccionada) {
                            idCarrera = $scope.delCarreras[te].id_carrera
                        };
                    };

                    for (var u = $scope.cursosAdd.length - 1; u >= 0; u--) {

                        if (valores.apj === $scope.cursosAdd[u].Nombre) {

                            console.log($scope.cursosAdd[u].CodCur)
                            $http.post('php/borrarGrupoCursoDirector.php', {
                                'Carrera': idCarrera,
                                'Curso': $scope.cursosAdd[u].CodCur
                            }).
            ...
            $('<p id="msgSuccess" class="alert alert-success">El curso de elimino correctamente</p>').insertBefore('#msjExi').delay(500).fadeOut();
        }
                        ...
                        for (var j = valorActualC.length - 1; j >= 0; j--) {
                            ...
                            for (var u = $scope.cursosAdd.length - 1; u >= 0; u--) {
                                ...
                                if (valorActualC[j].curso === $scope.cursosAdd[u].Nombre) {
                                    ...
                                    $http.post('php/agregarCursoCarreraDirector.php', {
                                        'Carrera': idCarrera,
                                        'Curso': $scope.cursosAdd[u].CodCur
                                    }).
                                    success(function(dataGrupo, status) {});
        ...
        $scope.del = function(grupos) {
            ...
            var f = $scope.este.datos.GruposDeCurso.indexOf(grupos);
            $scope.delet = function() {

                //-----------------------PHP Borrar----------------------
                $http.post('php/borrarGrupoDirector.php', {
                    'nombreGruZ': z
                }).
                success(function(dataGrupo, status) {});

                $http.post('php/borrarGrupoEstudiantesDirector.php', {
                    'nombreGruEz': z
                }).
                success(function(dataGrupo, status) {});

                $http.post('php/borrarGrupoProfesoresDirector.php', {
                    'nombreGruPrZ': z
                }).
                success(function(dataGrupo, status) {});
                //-----------------------PHP Borrar----------------------

                $scope.este.datos.GruposDeCurso.splice(f, 1);
        ...
        $scope.editCrus = function(inf) {
            ...
            for (var j = 0; j < valorActual.Estudiantes.length; j++) {
                $scope.estT.push({
                    nombre: valorActual.Estudiantes[j].NombreEstudiante,
            ...
            for (var k = 0; k < valorActual.Profesores.length; k++) {
                $scope.proT.push({
                    nombre: valorActual.Profesores[k].NombreProfesor,
        ...
        $scope.delTempProf = function(cs) {
                $http.post('php/editar_borrarProfesorGrupo.php', {
                    'GrupoPfr': cs.grupo,
                    'nombrePrf': cs.nombre
                }).
                success(function(dataGrupo, status) {})
        ...
        $scope.delTempEst = function(zc) {
                $http.post('php/editar_borrarEstudianteGrupo.php', {
                    'GrupoExt': zc.grupo,
                    'correoExt': zc.correoEss
                }).
                success(function(dataGrupo, status) {})
            ...
            if (count === 0) {
                ...
                $http.get('php/buscarGruposCurso.php').success(function(data) {
                    ...
                    if (ErrorNombre === 0) {
                        ...
                        // -----------------------PHP --------------------
                        $http.post('php/agregarGrupoCurso.php', {
                            'IdC': codCurz,
                            'nombreG': $scope.infoIngresada2
                        }).
                        success(function(dataGrupo, status) {})
                        ...
                        for (var g = valorActualE.length - 1; g >= 0; g--) {
                            $http.post('php/agregarEstudiantesGrupodirector.php', {
                                'IdCurso': codCurz,
                                'correoEst': arregloTemporalCorreos[g],
                                'GrupoEnt': $scope.infoIngresada2
                            }).
                            success(function(dataGrupo, status) {
                        ...
                        for (var u = valorActualP.length - 1; u >= 0; u--) {
                            $http.post('php/agregarProfesoresGrupodirector.php', {
                                'RolProf': pan[u],
                                'nombreProf': valorActualP[u].NombreProfesor,
                                'GrupoAsig': $scope.infoIngresada2
                            }).
                            success(function(dataGrupo, status) {})
                        };
                ...
                if (numeroPuest === 100) {
                    ...
                    $http.get('php/buscarRubricaDirector.php').success(function(das) {
                        ...
                        for (var w = 0; w < s; w++) {
                            ...
                            $http.post('php/editar_cambiarRubroRubrica.php', {
                                'valRub': num,
                                'nomRub': rubr,
                                'RubrNon': $scope.EditCursoId
                            }).
                            success(function(dataGrupo, status) {})
                        };
                    })
        ...
        $scope.delR = function(valores) {
            ...
            $scope.delRubbr = function() {
                ...
                //----------------PHP Borrar rubrica-----------//
                $http.post('php/borrarRubricaDirector.php', {
                    'rubNomb': valores.rubricaLista[0].NombreDeRubrica
                }).
                success(function(dataGrupo, status) {})
                ...
                $http.get('php/buscarRubrica.php').success(function(data) {
                    ...
                    for (var i = id.length - 1; i >= 0; i--) {
                        if (id[i].NombreRubrica === valores.rubricaLista[0].NombreDeRubrica) {
                            ...
                            $http.post('php/borrarRubrosRubrica.php', {
                                'idRub': parIdRub
                            }).
                            success(function(dataGrupo, status) {})
                        };
                ...
                $http.post('php/borrarRubricaCursoDirector.php', {
                    'nosis': valores.rubricaLista[0].NombreDeRubrica
                }).
                success(function(dataGrupo, status) {})
                ...
                if (total === 100) {
                    ...
                    $http.get('php/buscarRubrica.php').success(function(data) {
                        ...
                        if (errorNomb === 0) {
                            ...
                            $http.post('php/agregarRubricaDirector.php', {
                                'nombreRub': $scope.newNombreRubrica
                            }).
                            success(function(dataGrupo, status) {

                                $http.post('php/buscarCurso.php', {
                                    'data': $scope.newNombreGrupo
                                }).
                                success(function(dass, status) {

                                    $http.post('php/agregarRubricaCursoDirector.php', {
                                        'nRub': $scope.newNombreRubrica,
                                        'CuNob': $scope.newNombreGrupo
                                    }).
                                ...
                                $http.get('php/buscarRubricaDirector.php').success(function(das) {
                                    ...
                                    for (f--; f >= 0; f--) {
                                        ...
                                        $http.post('php/agregarRubrosDirector.php', {
                                            'nomb': rubr,
                                            'valr': num,
                                            'rubNomb': $scope.idCursoTem
                                        }).
                                        success(function(dataGrupo, status) {})
                                    };
                                });
...
//For the brave souls who get this far: You are awesome.
